...
function RegistrationWizard(){
  ...
  return <main className="reg6">
    <aside className="reg6-left">
      <img className="reg6-photo" src="/registration-hero.svg" alt="Competition athlete" />
    ...
    <section className="reg6-right">
      ...
      <div className="reg6-stage">
        <section className={`reg6-card reg6-form ${isDark?"dark":""}`}>
          ...
          {current==="Entry"&&<>
            <div className="reg6-choice-grid">{[["RX","RX","Advanced"],["Intermediate","INT","Competitive"],["Scaled","SCL","Open"]].map(([name,code,label])=><button key={name} className={`reg6-choice ${division===name?"active":""}`} onClick={()=>setDivision(name)}><span className="reg6-check">{division===name?"✓":""}</span><div className="reg6-choice-code">{code}</div><small>{label}</small></button>)}</div>
            <div className="reg6-format-grid"><button className={`reg6-format ${type==="INDIVIDUAL"?"active":""}`} onClick={()=>setType("INDIVIDUAL")}><span>01</span><div><strong>Individual</strong><small>One athlete. One entry.</small></div></button><button className={`reg6-format ${type==="TEAM"?"active":""}`} onClick={()=>setType("TEAM")}><span>02</span><div><strong>Team</strong><small>Captain controls the roster.</small></div></button></div>
          ...
          {(current==="Captain"||current==="Athlete")&&<><div className="reg6-row"><div><label>First name</label><input value={captain.firstName} onChange={e=>updateCaptain("firstName",e.target.value)}/></div><div><label>Last name</label><input value={captain.lastName} onChange={e=>updateCaptain("lastName",e.target.value)}/></div></div><div className="reg6-row"><div><label>Email</label><input type="email" value={captain.email} onChange={e=>updateCaptain("email",e.target.value)}/></div><div><label>Phone</label><input value={captain.phone} onChange={e=>updateCaptain("phone",e.target.value)}/></div></div><div className="reg6-row"><div><label>Gender</label><select value={captain.gender} onChange={e=>updateCaptain("gender",e.target.value)}><option value="">Select</option><option>Male</option><option>Female</option></select></div><div><label>Date of birth</label><input type="date" value={captain.birthDate} onChange={e=>updateCaptain("birthDate",e.target.value)}/></div></div><div className="reg6-row"><div><label>Gym / Affiliate</label><input value={captain.gym} onChange={e=>updateCaptain("gym",e.target.value)} placeholder="Optional"/></div><div><label>Shirt size</label><select value={captain.shirtSize} onChange={e=>updateCaptain("shirtSize",e.target.value)}><option>XS</option><option>S</option><option>M</option><option>L</option><option>XL</option><option>2XL</option></select></div></div></>}

          {current==="Roster"&&<div className="reg6-roster"><div className="reg6-list"><div className="reg6-captain"><span>01</span><div><strong>{captain.firstName||"Captain"} {captain.lastName}</strong><small>Primary contact</small></div><em>YOU</em></div>{teammates.map((a,i)=><div className="reg6-athlete" key={a.id}><span>{String(i+2).padStart(2,"0")}</span><div><strong>{a.firstName||`Athlete ${i+2}`} {a.lastName}</strong><small>{a.email||"Details not complete"}</small></div><button onClick={()=>setEditing(a.id)}>Edit</button></div>)}{teammates.length<4&&<button className="reg6-add" onClick={addMate}><b>+</b><span><strong>Add teammate</strong><small>Open another roster spot</small></span></button>}</div>{teammates.filter(a=>a.id===editing).map(a=><div className="reg6-edit" key={a.id}><div className="reg6-edit-head"><h3>Athlete {teammates.findIndex(t=>t.id===a.id)+2}</h3>{teammates.length>1&&<button onClick={()=>removeMate(a.id)}>Remove athlete</button>}</div><div className="reg6-row"><div><label>First name</label><input value={a.firstName} onChange={e=>updateMate(a.id,"firstName",e.target.value)}/></div><div><label>Last name</label><input value={a.lastName} onChange={e=>updateMate(a.id,"lastName",e.target.value)}/></div></div><div className="reg6-row"><div><label>Email</label><input value={a.email} onChange={e=>updateMate(a.id,"email",e.target.value)}/></div><div><label>Gender</label><select value={a.gender} onChange={e=>updateMate(a.id,"gender",e.target.value)}><option value="">Select</option><option>Male</option><option>Female</option></select></div></div><div className="reg6-row"><div><label>Gym / Affiliate</label><input value={a.gym} onChange={e=>updateMate(a.id,"gym",e.target.value)} placeholder="Optional"/></div><div><label>Shirt size</label><select value={a.shirtSize} onChange={e=>updateMate(a.id,"shirtSize",e.target.value)}><option>XS</option><option>S</option><option>M</option><option>L</option><option>XL</option><option>2XL</option></select></div></div></div>)}</div>}

          {current==="Safety"&&<><div className="reg6-row"><div><label>Emergency contact</label><input value={emergencyName} onChange={e=>setEmergencyName(e.target.value)}/></div><div><label>Emergency phone</label><input value={emergencyPhone} onChange={e=>setEmergencyPhone(e.target.value)}/></div></div><div className="reg6-waiver"><strong>Participant waiver placeholder</strong>The final Training Day Games participation waiver, assumption of risk, media release and event policies will appear here before registration opens.</div><label className="reg6-checkrow"><input type="checkbox" checked={waiver} onChange={e=>setWaiver(e.target.checked)}/><span>I have read and agree to the Training Day Games waiver and event policies.</span></label></>}

          {current==="Review"&&<><div className="reg6-review"><div className="reg6-review-card"><div><span>Competition</span><strong>{division} · {type==="TEAM"?"Team":"Individual"}</strong></div><button onClick={()=>go(0)}>Edit</button></div><div className="reg6-review-card"><div><span>{type==="TEAM"?"Captain":"Athlete"}</span><strong>{captain.firstName||"—"} {captain.lastName}</strong></div><button onClick={()=>go(1)}>Edit</button></div>{type==="TEAM"&&<div className="reg6-review-card"><div><span>Roster</span><strong>{teammates.length+1} athletes · {teamName||"Team name not set"}</strong></div><button onClick={()=>go(2)}>Edit</button></div>}<div className="reg6-review-card"><div><span>Emergency contact</span><strong>{emergencyName||"—"}</strong></div><button onClick={()=>go(type==="TEAM"?3:2)}>Edit</button></div></div>{error&&<div className="reg6-error">{error}</div>}</>}

          <div className="reg6-actions">{step>0?<button className="reg6-back" onClick={()=>go(step-1)}>← Back</button>:<span/>}{current!=="Review"?<button className="reg6-next" onClick={()=>go(step+1)}>Continue <span>→</span></button>:<button className="reg6-pay" onClick={checkout} disabled={loading}>{loading?"Opening checkout...":"Continue to payment"}<span>→</span></button>}</div>
        </section>
      </div>
    </section>
  </main>
